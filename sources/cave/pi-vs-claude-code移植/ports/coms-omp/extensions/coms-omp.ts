/**
 * coms — Peer-to-peer messaging between OMP agents on the same machine
 *
 ...
 * ~/.omp/coms/projects/<project>/agents/<name>.json.
 *
 ...
 *
 * Usage: omp -e extensions/coms-omp.ts
 */

import type { ExtensionAPI, ExtensionContext, Theme } from "@oh-my-pi/pi-coding-agent";
import { DynamicBorder } from "@oh-my-pi/pi-coding-agent";
import { Text, Container, truncateToWidth, visibleWidth, wrapTextWithAnsi } from "@oh-my-pi/pi-tui";
...
const PING_INTERVAL_MS = Number(process.env.OMP_COMS_PING_INTERVAL_MS) || 10_000;
...
function readCliFlags(pi: ExtensionAPI): CliFlags {
	...
	// OMP CLI parser accepts them; here we just read them back.
...
export default function (pi: ExtensionAPI) {
	...
	// Without these, OMP rejects the invocation with "Unknown options:
	// --cname, --project, ..." before this extension's hooks ever fire.
	// Agent name flag is `--cname`: keeps this extension away from harness/session `--name` semantics.
	pi.registerFlag("cname", {
		description: "Override coms agent name (otherwise from frontmatter or auto-generated). Distinct from the harness/session --name.",
		type: "string",
		default: undefined,
	});
	...
	pi.registerTool({
		...
		label: "Coms Await",
		description:
			"Block until a pending coms_send reply lands or the timeout fires. Default timeout 30 minutes (OMP_COMS_TIMEOUT_MS).",
		parameters: Type.Object({
			msg_id: Type.String({ description: "msg_id returned by coms_send." }),
