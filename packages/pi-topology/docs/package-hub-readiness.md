...
| `topology_spawn_role` | READY_LOCAL | 支持 `initial_prompt` / `log_path`，launch script tee 落证；2026-06-16 MiniMax/Ghostty clean dogfood 通过 `mode=launch` 启动 HQ 并写入同一 runtime/outbox；2026-06-17 当前 Mac 改用 direct generated script lane 作为 E2E gate | Ghostty GUI command probe 在当前 Mac 不稳定；`launch_requested` 不可当作 alive |
...
| 实际多 session Ghostty/Pi role smoke | READY_LOCAL_WITH_CURRENT_MAC_CAVEAT | 2026-06-16 MiniMax M3 + Ghostty clean dogfood 覆盖 `hq` / `repair` / `runner` / `oracle` / `librarian` / `scott`，每个角色写入非空 local packet；run root `/tmp/pi-topology-dogfood-minimax-clean-2026-06-16` | 2026-06-17 当前 Mac 的 unattended Ghostty GUI lane 暂不作为 gate；仅验证 local JSONL transport，不验证 HTTP/SSE |
...
- 当前 Mac 上，只有在最小探针 `open -n -a Ghostty --args -e /usr/bin/touch /tmp/probe` 稳定且无 failed-command window / abnormal-process log 后，才重新把 Ghostty GUI lane 作为 E2E gate。此前使用 direct generated script lane。
